...
class One_Stage_Mock_Provider(LLMProvider):
    ...
    def generate(self, ctx: Context, context_history: list[Message], available_tools, stream=False):
        self.turn += 1
        if self.turn == 1:
...
def launch():
    ...
    engine = AgentEngine(OpenAIProvider.create_deepseek_provider(), Mock_Registry(), "./", True, True)
